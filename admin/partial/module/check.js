...
.controller("moduleCheckCtrl",function ($scope,$rootScope,$moduleAction,$http,$stateParams,$state)
{
    ...
    $scope.last = function ()
    {
        $http.post($moduleAction.moduleLast,{module_id:$stateParams.id,re:"false"})
            .success(function (result) {
                if (result.status) {
                    $scope.result=result;
                    $scope.success=true;
